<div align="center">
  ...
  <p align="center">
    YOLOv8-based shelf product recognition and annotation system<br />
    基于 YOLOv8 的货架商品识别与标注系统
  </p>
</div>
...
- **GitHub Actions CI/CD** -- Automated build pipeline produces Windows NSIS installers and macOS DMGs with optional codesigning and notarization.
...
| Desktop Shell  | pywebview (native WebView), with browser fallback                          |
...
# 4. Download the base YOLOv8n model
#    (it will download automatically on first run, or place yolov8n.pt in backend/)

# 5. Start the server
python backend/start.py
```

The application will open in your default browser at `http://127.0.0.1:8000`. If pywebview is available, it launches as a native desktop window instead.
...
The REST API is served at the root of the application. Key endpoints:

| Endpoint                    | Method | Description                              |
|-----------------------------|--------|------------------------------------------|
| `/infer/health`             | GET    | Health check                             |
| `/infer/upload`             | POST   | Upload an image and run inference        |
| `/datasets/list`            | GET    | List all datasets                        |
| `/datasets/create`          | POST   | Create a new dataset (from ZIP or path)  |
...
- Python 3.10+
- PyInstaller
- For Windows: NSIS (for the installer)
- For macOS: Xcode command-line tools (for codesigning)
...
The packaged application includes all data assets (frontend, base model, etc.) as bundled resources.
...
When running in development mode (not frozen by PyInstaller), data is stored in `backend/`.
...
- [Ultralytics YOLOv8](https://github.com/ultralytics/ultralytics) for the object detection engine
- [Annotorious](https://annotorious.dev/) for the image annotation toolkit
- [Vue 3](https://vuejs.org/) and [Element Plus](https://element-plus.org/) for the frontend framework
